...
const HARDCODED_PRODUCTS: Product[] = [
  ...
  },
  {
    ...
    productId: "RING002",
    description:
      "Make a bold statement with this exquisite piece. A vibrant emerald surrounded by delicate filigree work in yellow gold.",
    price: 38999,
    originalPrice: 42999,
    onSale: true,
    isOutOfStock: false,
...
export default function ImageFanDynamic() {
    ...
    const fetchFeaturedProducts = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/user/ornaments`,
        );
        const data = await res.json();

        const featuredOnly = data.ornaments
          .filter((p: Product) => p.isFeatured && !p.isVariant)
          .map((p: Product) => {
            // Match by productId and carry over the url from HARDCODED_PRODUCTS
            const hardcoded = HARDCODED_PRODUCTS.find(
              (h) => h.productId === p.productId,
            );
            return { ...p, url: hardcoded?.url || "/products" };
          });

        setProducts(featuredOnly);
      } catch (error) {
        console.error("Error fetching featured products:", error);
        // Keep hardcoded products as fallback if API fails
        setProducts(HARDCODED_PRODUCTS);
      }
    };

    fetchFeaturedProducts();
    const interval = setInterval(fetchFeaturedProducts, 8000);

    return () => clearInterval(interval);
  }, []);

  // ✅ Prevent index overflow when featured list changes
  useEffect(() => {
  ...
  return (
    ...
    >
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="relative h-[70%] overflow-hidden bg-gradient-to-br from-slate-100 via-white to-slate-50">
                    ...
                    <div className="absolute top-3 right-3">
                      <span
                        className={`inline-block backdrop-blur-md bg-white/90 text-slate-700 px-3 py-1.5 rounded-full font-semibold shadow-lg border border-white/20 ${isMobile ? "text-[10px]" : "text-xs"}`}
                      >
                        {product.category}
                      </span>
                    </div>
                  ...
                  <div
                    className={`flex flex-col justify-between bg-white ${isMobile ? "px-3 py-2" : "px-4 py-3"} h-[25%]`}
                  >
                    {/* Product Name */}
                    <h3
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-1.5">
                        <svg
                          className={`${isMobile ? "w-3 h-3" : "w-4 h-4"} text-amber-400 fill-current`}
                          viewBox="0 0 20 20"
                        >
                      ...
                      <span
                        className={`text-slate-400 font-medium group-hover:text-blue-600 transition-colors duration-300 ${isMobile ? "text-[10px]" : "text-xs"}`}
                      >
      ...
      {animationPhase === "carousel" && products.length > 0 && (
        <div
          className="flex items-center gap-4"
          style={{ zIndex: 30, marginTop: isMobile ? "12px" : "20px" }}
        >
          <button
            onClick={handlePrev}
            onMouseEnter={() => setPaused(true)}
            onMouseLeave={() => setPaused(false)}
            className="flex items-center justify-center rounded-full bg-white border border-slate-200 shadow-md hover:shadow-lg hover:bg-slate-50 active:scale-95 transition-all duration-200"
...
// import React, { useState, useEffect } from 'react';
// import { motion, TargetAndTransition, VariantLabels, Variants,AnimatePresence } from 'framer-motion';
...
//       const featuredOnly = data.ornaments.filter(
//         (p: Product) => p.isFeatured && !p.isVariant
//       );
