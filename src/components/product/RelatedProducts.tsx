...
const RelatedProducts: React.FC<RelatedProductsProps> = ({ products }) => {
  ...
  // ✅ FINAL PRICE FUNCTION (FIXED)
  const getDisplayPrice = (product: Product) => {
    const currency = selectedCountry?.currency || "INR";
    const { symbol } = currencyRates[currency] || currencyRates.INR;

    // 🔥 1. ALWAYS prefer backend computed price
    if (product.displayPrice) {
      return `${symbol}${Number(product.displayPrice).toLocaleString()}`;
    }

    // 🔥 2. Then currency-based prices
            ...
            >
              ...
              <div className="relative aspect-square overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex flex-col space-y-1">
                  ...
                  {product.isBestSeller && (
                    <span className="px-2 py-1 bg-[#9a8457] text-white text-xs rounded-full">
                      Best Seller
                    </span>
                  )}
                  {(product.discount ?? 0) > 0 && (
