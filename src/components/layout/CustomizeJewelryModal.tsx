  ...
  if (toast) {
	...
	const handleSubmit = async () => {
		...
		if (
			...
			!customOptions.email ||
			!customOptions.phone ||
			!customOptions.inspiration ||
			!customOptions.specialRequests
		) {
			setErrorMessage('Please fill all required fields');
			return;
		}

		if (!emailRegex.test(customOptions.email)) {
			...
			<div className="relative bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden max-h-[95vh] sm:max-h-[90vh] flex flex-col">
				...
				<div className="p-3 sm:p-6 flex-1 overflow-y-auto text-xs sm:text-base leading-relaxed">
					...
					{currentStep === 1 && !showThankYou && (
						<div>
							<div className="text-center mb-4 sm:mb-6">
								<h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Your Vision</h3>
								<p className="text-sm sm:text-base text-gray-600">
									Share your inspiration and any special requests
								</p>
							</div>
					...
					{currentStep === 2 && !showThankYou && (
						<div onKeyDown={handleKeyPress}>
							<div className="text-center mb-4 sm:mb-6">
								<h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Your Details</h3>
								<p className="text-sm sm:text-base text-gray-600">
						...
						<div className="flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-0">
							...
							{currentStep === 1 ? (
								<button
									...
									disabled={
										!customOptions.inspiration.trim() || !customOptions.specialRequests.trim()
									}
