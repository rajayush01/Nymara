...
const CartPage = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const orderPayload = {
        products: activeCart.map((item) => {
          const price = item.prices?.[selectedCountry.currency]?.amount ?? item.price ?? 0;
          const making = item.makingChargesByCountry?.[selectedCountry.currency]?.amount ?? 0;
          const unitPrice = (price + making) * item.quantity;
          return {
            productId: item._id,
            variant: item.metalType || "Yellow Gold",
            quantity: item.quantity,
            unitPrice, // total price for this line item (unit price * qty)
          };
        }),
        deliveryAddress: {
          name: `${shippingInfo.firstName} ${shippingInfo.lastName}`,
