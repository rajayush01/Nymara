					...
					>
						{isSuccess ? (
							...
							<svg width="18" height="18" viewBox="0 0 18 18" fill="none">
								...
								<path d="M9 5.5v4M9 12.5v.5" stroke="#dc2626" strokeWidth="2" strokeLinecap="round" />
							</svg>
						)}
					</div>
...
const LoginPage: React.FC<LoginPageProps> = ({ onSwitchToSignup }) => {
	...
	const handleForgotPassword = async () => {
		...
		try {
			const response = await axios.post(`${API_URL}/api/user/forgetpassword`, { email: forgotEmail });
				...
				<div className="glass-card rounded-3xl overflow-hidden gentle-breathe magnetic relative">
					...
					<div className="p-8 text-center relative">
						...
						<div className="relative z-10">
							...
							<h1 className="text-3xl font-light text-slate-800 mb-2 tracking-wide">
								<span className="bg-[#9a8457] bg-clip-text text-transparent">Welcome</span>
							</h1>
					...
					<form onSubmit={handleSubmit} className="px-8 pb-8 space-y-6">
						...
						<div className="relative group">
							<label className="block text-slate-700 text-sm mb-2 font-medium">Email</label>
						...
						{/* Password field */}
						<div className="relative group">
							<label className="block text-slate-700 text-sm mb-2 font-medium">Password</label>
							<div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10 mt-8">
								<Lock
									className="h-4 w-4 text-slate-400 transition-colors duration-300"
									style={{
						...
						<div className="flex items-center justify-between">
							<label className="flex items-center cursor-pointer group">
								...
								/>
								<div
									className={`w-5 h-5 border-2 rounded flex items-center justify-center mr-3 transition-all duration-200 bg-white ${
										rememberMe ? 'border-slate-400' : 'border-slate-300'
									}`}
								>
									<span
										className={`text-slate-400 text-xl font-bold leading-none transition-transform duration-200 ${
							...
							{showForgotModal && (
								...
								>
									...
									>
										<h2 className="text-xl font-semibold mb-3 text-slate-800">Reset Password</h2>
